...
Il y a normalement un bar d'étage par section ; cependant, comme il n'existe que 15 bars d'étage pour 16 sections, il y a forcément un bar d'étage partagé entre deux sections. Mais ne t'inquiète pas ; plus on est de fous, plus on rit !
...
\begin{itemize}
	...
	\item un panneau \menu{Suggestion d'aliments} pour suggérer des aliments à tes respos bar pour la prochaine appro.
...
\paragraph{Menus} Si tu te retrouves à logguer périodiquement les mêmes aliments dans les mêmes quantités, tu peux créer un menu prédéfini pour aller plus vite. Il suffit d'aller sur \menu{Préférences>Mes menus>Créer}. Tu peux donner à ton menu un nom, et il te suffira de taper ce nom dans la Magic Bar\texttrademark{} pour le logguer en une seule fois.
...
\paragraph{Signaler un bug} En cliquant sur \menu{J'ai une remarque}, tu peux envoyer un message à l'équipe de développement de Chocapix qui s'empressera de prendre en compte ton avis sur une erreur dans le site ou une fonctionnalité manquante. Utilise cette fonction pour que le site bar soit encore mieux ! Les messages sont directement relayés sur le chat des développeurs de Chocapix, ils ne tombent pas dans l'oreille d'un sourd.

\subsection{Classements et statistiques}

Au delà de l'historique des consommations du bar, Chocapix t'offre la possibilité de consulter énormément de statistiques sur la consommation de ton bar. Ainsi, sur la fiche d'un aliment, les onglets \menu{Graphiques} et \menu{Classements} te permettent de visualiser respectivement la consommation de l'aliment au cours du temps (ou en moyenne sur les heures de la journée, du mois, etc.) ou les personnes qui consomment le plus cet aliment. Les mêmes onglets sont présents sur les fiches des utilisateurs.
...
Dans l'onglet \menu{Classements} de la barre de navigation à gauche, tu trouveras trois classements des utilisateurs de ton bar :
\begin{itemize}
...
\subsection{Interactions avec les respos bar}


\paragraph{Actualités} Régulièrement tes respos bar publieront sur la page d'accueil des actualités qui concernent le barf d'étage. Événements, coups de gueule, n'oublie pas de regarder régulièrement les annonces pour te tenir au courant.

\paragraph{Suggestion d'aliments} Pour aider tes respos bar à mieux commander leurs appros, la page d'acceuil du site comporte un encart \emph{Nouveaux aliments souhaités} où tu peux soit ajouter une suggestion soit upvoter une suggestion déjà faite en cliquant sur le pouce vert.

\subsection{Astuces}

\begin{enumerate}
	\item On peut se déconnecter en appuyant deux fois sur la touche \keys{Echap}.
	...
	\item Tu peux voir les horaires où quelqu'un consomme le plus en allant sur sa page, onglet \menu{Graphiques}, \menu{Type de graphique} : Moyenne, \menu{Date de début} : ouverture de ton bar.
	\item La MagicBar™ comprend les expressions mathématiques (sans espaces) : \texttt{(500-187)/3 pâtes}.
	\item Tu peux faire une bouffe à plusieurs sans jamais utiliser la souris : \texttt{aj 2 pizza}, \keys{Entrer}, \texttt{2 café}, \keys{Entrer}, \keys{Tab}, \texttt{nom-de-la-personne}, \keys{Entrer}, \keys{Tab}, \keys{Entrer}.
...
\paragraph{Aliments suggérés} La première chose à faire est d'aller regarder les suggestions des utilisateurs du bar sur \menu{Administration>Aliments>Aliments suggérés}. Tu peux dans ce menu supprimer les suggestions que tu as pris en compte afin de ne pas avoir de doublons.

\paragraph{Estimation automatique des quantités} Tu pourras trouver dans \menu{Administration>Aliments>Estimation des quantités à acheter} une interface permettant de calculer la quantité de chaque aliment à acheter pour la prochaine appro en fonction de la consommation des utilisateurs depuis la dernière appros. Cette fonctionnalité ets encore expérimentale et ne marche pas toujours mais permet de confirmer des tendances de consommation pour ne pas acheter trop (aliments jetés) ou pas assez (utilisateurs mécontents).
...
\begin{enumerate}
	...
	\item Il faut maintenant signifier qu'il ne reste aucun autre aliment autre que ceux que tu viens d'entrer dans le bar. Pour cela, clique sur le bouton \menu{Épuisé} de tous les aliments restants dans l'encart de droite.
\end{enumerate}
...
\paragraph{Mailing list} Pour toujours plus de spam, tu pourras obtenir une mailing list de tous les utilisateurs de ton bar sur \menu{Administration>Actualités>Envoyer un message aux utilisateurs}.

\subsection{Trézobar}

Le trésorier du bar gère la trésorerie du bar, qui est un peu subtile. Tout est détaillé sur le WikiX à la page \url{http://wikix.polytechnique.org/Respo_bar/Trézobar}. Chocapix ne gère pas la comptabilité du bar, celle-ci doit donc etre tenue à part sur un Excel. Le plus important est de noter toutes les rentrées et sorties sur le compte bancaire du bar, car c'est cela qui indiquera à la fin l'état des finances.

\paragraph{Indicateurs financiers} Chocapix donne deux informations essentielles à la comptabilité du bar dans l'onglet \menu{Administration>Comptabilité>État du bar} :
\begin{itemize}
	\item la somme algébrique des soldes de tous les comptes utilisateurs ;
	\item la valeur totale de tous les stocks d'aliments déclarés dans Chocapix ;
\end{itemize}

Si la première valeur reflète toujours la réalité, la deuxième est généralement fausse à cause des vols et oublis de logs. D'où la nécessité d'un inventaire total pour mettre au clair les comptes du bar.
